...
const Community = () => {
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-6 px-4 sm:px-6">
            <div>
                <h1 className="text-2xl sm:text-3xl font-bold mb-2">Muro Social</h1>
                <p className="text-sm sm:text-base text-gray-600">Comparte tus experiencias con la comunidad</p>
            </div>
...
const PostCard = ({ post, currentUser, onLike, onDelete, onComment }) => {
    ...
    return (
        <div className="bg-white rounded-lg shadow p-4 sm:p-6">
            ...
            <div className="flex items-start justify-between mb-4">
                <div className="flex items-center space-x-2 sm:space-x-3 flex-1 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                        <div className="font-semibold text-sm sm:text-base truncate">{post.user.name}</div>
            ...
            {/* Post Content */}
            <p className="text-sm sm:text-base text-gray-800 mb-4 whitespace-pre-line break-words">{post.content}</p>
            ...
            <div className="flex items-center space-x-4 sm:space-x-6 pt-4 border-t">
                <button
                    ...
                    className="flex items-center space-x-1 sm:space-x-2 text-gray-600 hover:text-primary"
                >
                    <svg className="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="mt-4 pt-4 border-t space-y-4">
                    ...
                    {currentUser && (
                        <form onSubmit={handleSubmitComment} className="flex flex-col sm:flex-row gap-2">
                            ...
                            />
                            <Button type="submit" disabled={!commentText.trim()} className="w-full sm:w-auto">
                                Enviar
                            </Button>
                        </form>
                    )}
                            ...
                            {post.comments.map(comment => (
                                <div key={comment.id} className="flex space-x-2">
                                    <div className="flex-1 bg-gray-50 rounded-lg p-3">
                                        <div className="font-semibold text-xs sm:text-sm">{comment.user.name}</div>
